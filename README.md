...
|---------------|-------------|-----------------------------------------|
| Linux x86-64  | tested      | gcc 11, gcc 13                          |
| macOS         | tested      | Apple clang 15, same flags work         |
| Windows MinGW | tested      | MinGW-w64, output binary is ringbuf.exe |
| WSL2          | tested      | same as Linux                           |
| ARM Cortex-M  | not tested  | designed for it, not cross-compiled     |
| AVR           | not tested  | the AND trick is especially useful here |

---

## Known Limitations

Not ISR-safe as written. In a real firmware project you would wrap rb_write()
with an interrupt disable/enable pair, or use an atomic exchange, depending on
your target. This is a single-threaded demo so none of that is needed here.

BUFFER_SIZE is a compile-time constant. Change the #define and recompile if
you need a different size -- just keep it a power of 2 or the BUFFER_MASK trick
breaks. Dynamic allocation is deliberately out of scope for a bare-metal utility.

Only handles uint8_t. If you need 16-bit or 32-bit elements you'd need a
